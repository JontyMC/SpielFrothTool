...
const hallFactorConst = 3509
const defaultSize = 200
const itemsOnCircle = 5
const startAngle = -(1.8 * Math.PI) / itemsOnCircle

const hallFactorByHallId = hallArray.reduce((acc, hall) => {
    acc[hall.id] = hall.width / hallFactorConst
    return acc
}, <{ [hallId: string]: number }>{})
const hallFactorByBoothId = boothArray.reduce((acc, booth) => {
    acc[booth.id] = hallFactorByHallId[booth.hallId]
    return acc
}, <{ [hallId: string]: number }>{})

export function getDefaultSize(game: Game) {
    const size = defaultSize * (hallFactorByBoothId[game.boothId] ?? 1)
    return size
}

function getInitPosition(boothId: string, index: number) {
    const booth = booths[boothId] ?? {}
    const hallFactor = hallFactorByBoothId[boothId]
    const factoredDefaultSize = defaultSize * hallFactor
    const radiusIncrement = defaultSize * 1.3 * hallFactor
    if (booth) {
        ...
        return { x, y, width: factoredDefaultSize, height: factoredDefaultSize }
    }
}
